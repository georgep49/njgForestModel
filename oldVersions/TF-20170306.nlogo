...
to init-patch
  ...
  set species rnd-roulette (n-values n-species [? + 1]) start-abund
...
to thin-regenbank
   ...
   [
     set seedlings map [round ? * (1 - inhibit)] seedlings
   ]
end
...
to-report rnd-roulette [values probabilities]
  ...
  let state first rnd:weighted-one-of-list pairs [ last ?]
